...
public class CartScreenController {
    ...
    private FilteredList<Media> cartFilter;
    ...
    public void initialize() {
        ...
        cartFilter = new FilteredList<Media>(this.cart.getItemsOrdered(), s -> true);
        tblMedia.setItems(cartFilter);
    ...
    private void showFilteredMedia(String filter) {
        if (filter == null || filter.length() == 0) {
            cartFilter.setPredicate(s -> true);
        } else {
            if (filterByID) {
                try {
                    cartFilter.setPredicate(s -> s.getId() == Integer.parseInt(filter));
                } catch (NumberFormatException e) {
                }
            } else {
                cartFilter.setPredicate(s -> s.getTitle().toLowerCase().contains(filter));
            }
        }
    }
